  ...
  <div class="col-md-6">
    ...
    <div class="input-group input-group-sm py-1">
      ...
      <template v-if="field.type == 'input'">
        <input :id="field.props.name" class="form-control" :type="field.props?.type" v-model="modelValue" />
      </template>

      <template v-if="field.type == 'select'">
        <select :id="field.props.name" class="form-select" v-model="modelValue">
          <option selected>Выберите</option>
          <option v-for="(option, Opindex) in field.options" :key="Opindex" :value="option.value">
            {{ option.text }}
          </option>
        </select>
      </template>
...
<script setup lang="ts">
  ...
  import { FieldType, QueryType } from '@/types/fiields';
